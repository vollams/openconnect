...
total_hysteria=$(ss -unp 2>/dev/null | grep -c ":${hysteria_port:-5666} " || echo 0)
. /root/.ports
. /etc/.db-base 2>/dev/null || . /etc/openvpn/login/config.sh
...
xray_online=$(mysql --ssl-verify-server-cert=OFF -u $DB_USER -p$DB_PASS -D $DB_NAME -h $DB_HOST -sN -e "SELECT COUNT(*) FROM users WHERE is_connected_xray=1 AND active_address='$ip'" 2>/dev/null || echo 0)
...
UPDATE server_list SET
  ...
  xray_xhttp_tls   = '$(escape_for_sql "$xray_xhttp_tls")',
  xray_xhttp_ntls  = '$(escape_for_sql "$xray_xhttp_ntls")',
  xray_status      = '$(escape_for_sql "$xray")',
  ...
  squid            = '$(escape_for_sql "$squid_port")',
  socket           = '$(escape_for_sql "$socket_port")',
  online           = '$(escape_for_sql "$xray_online")'
WHERE server_ip = '$(escape_for_sql "$ip")'
;
SQL
)
